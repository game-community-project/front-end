...
const TeamInfo: React.FC = () => {
    ...
    const fetchTeamInfo = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const adminResponse = await axios.get(`http://51.21.48.160:8080/api/users/profile/${response.data.data.teamAdminId}`);
          if (adminResponse.status === 200) {
            setAdminName(adminResponse.data.data.nickname);
          }

          const usersResponse = await axios.get(`http://51.21.48.160:8080/api/teams/${teamId}/users`);
          ...
          if (accessToken) {
            ...
            const isTeamAdminResponse = await axios.get(`http://51.21.48.160:8080/api/teams/${teamId}/admin`, config);
  ...
  const deleteTeam = async (teamId: string) => {
    try {
      ...
      const res = await axios.delete(`http://51.21.48.160:8080/api/teams/${teamId}`, config);

...
function CreatePost() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      formData.append('requestDto', JSON.stringify(requestData));
      formData.append('gameType', gameType);
      formData.append('gameName', gameName);
      formData.append('boardName', boardName);

      const response = await fetch(`http://localhost:8080/api/posts?gameType=${gameType}&gameName=${gameName}&boardName=${boardName}`, {
        method: 'POST',
        headers: {
          'Access': `${accessToken}`,
        },
